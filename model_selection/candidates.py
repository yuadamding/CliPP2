...
def _evaluate_candidate(
    *,
    ...
    row: dict[str, float | int | str | bool] = {
        ...
        "activity_passes": int(fit.activity_passes),
        "analytic_adjoint_passes": int(fit.analytic_adjoint_passes),
        "column_scan_passes": int(fit.column_scan_passes),
...
def _evaluate_partition_candidate(
    *,
    ...
    cem_iterations_value = -1
    if canonical_score_name == "marginal_bic":
        marginal_loglik_value, cem_iterations_value = _cem_stabilized_marginal_loglik(
            data,
            labels,
            fit_options=fit_options,
            hint_phi=phi,
            initial_refit=candidate.refit,
        )
    bic, classic_bic, extended_bic, partition_icl = _selection_score_value(
        loglik=loglik,
