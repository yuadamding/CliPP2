...
def _mutation_region_best_two_from_candidates(
    ...
    *,
    ...
    candidate_array = candidate_array[
        (candidate_array >= float(lower) - 1e-12)
        & (candidate_array <= float(upper) + 1e-12)
    ]
    ...
    if candidate_array.size == 0:
        fallback = (
            float(lower)
            if hint is None
            else _project_to_interval(float(hint), float(lower), float(upper))
        )
...
def _mutation_region_loss_grid_numpy(
    ...
    *,
    ...
    log_minor = (
        alt * np.log(prob_minus) + nonalt * np.log1p(-prob_minus) + log_prior_minor
    )
    log_major = (
        alt * np.log(prob_plus) + nonalt * np.log1p(-prob_plus) + log_prior_major
    )
...
def _batched_log_grid(
    ...
    *,
    ...
    lower_t = torch.as_tensor(
        np.asarray(lower, dtype=np.float64), dtype=runtime.dtype, device=runtime.device
    )
    upper_t = torch.as_tensor(
        np.asarray(upper, dtype=np.float64), dtype=runtime.dtype, device=runtime.device
    )
    t = torch.linspace(
        0.0, 1.0, steps=int(num_points), dtype=runtime.dtype, device=runtime.device
    )
    return torch.exp(
        torch.log(lower_t).unsqueeze(-1)
        + (torch.log(upper_t) - torch.log(lower_t)).unsqueeze(-1) * t
    )
...
def compute_pooled_observed_data_start(
    ...
    *,
    ...
    for region_idx in range(data.num_regions):
        lower = float(eps)
        upper = float(
            np.min(np.asarray(data.phi_upper[:, region_idx], dtype=np.float64))
        )
        hint = None
        if beta_hints is not None:
            hint = float(
                np.median(np.asarray(beta_hints[:, region_idx], dtype=np.float64))
            )
        ...
        right = float(
            upper if best_index == grid_np.shape[0] - 1 else grid_np[best_index + 1]
        )
...
def compute_scalar_well_start_bank(
    ...
    *,
    ...
    secondary = np.asarray(
        secondary_wells if secondary_wells is not None else secondary, dtype=np.float64
    )
    ...
    for start in starts:
        ...
        if any(
            np.allclose(start_array, retained, rtol=0.0, atol=1e-8)
