...
def process_one_file_bundle(
    ...
    if not search_df.empty and "is_selected_best_row" in search_df.columns:
        selected_mask = (
            search_df["is_selected_best_row"].astype(bool).to_numpy(dtype=bool)
        )
    ...
    selected_lambda_value = (
        float(best_fit.lambda_value) if selected_lambda_applicable else np.nan
    )
    selected_lambda_representative = (
        np.nan
        if not selected_lambda_applicable
        or selection_result.selected_lambda_representative is None
        else float(selection_result.selected_lambda_representative)
    )
    if not search_df.empty:
        if "lambda_applicable" in search_df.columns:
            lambda_applicable_mask = (
                search_df["lambda_applicable"]
                .fillna(True)
                .astype(bool)
                .to_numpy(dtype=bool)
            )
        ...
        tested_lambda_values = search_df.loc[lambda_applicable_mask, "lambda"].to_numpy(
            dtype=float
        )
    ...
    tested_lambda_min = (
        float(np.min(tested_lambda_values)) if tested_lambda_values.size else np.nan
    )
    tested_lambda_max = (
        float(np.max(tested_lambda_values)) if tested_lambda_values.size else np.nan
    )
    ...
    def _timing_max(column: str) -> float:
        ...
        return (
            float(numeric.max(skipna=True)) if numeric.notna().any() else float(np.nan)
        )
    ...
    selected_model_token = (
        "selected_lambda" if selected_lambda_applicable else "selected_model"
    )
    selected_ari_source = (
    ...
    best_bic_refit_global = bool(
        _artifact_value("bic_refit_global_optimum_certified", False)
    )
    ...
    summary = {
        ...
        "selected_candidate_pool_source": _selected_str(
            "candidate_pool_source", "raw_fused_lambda_path"
        ),
        "selected_partition_candidate_source": _selected_str(
            "partition_candidate_source", ""
        ),
        "selected_partition_candidate_requested_K": _selected_int(
            "partition_candidate_requested_K", -1
        ),
        "loglik": float(best_fit.loglik),
        "summary_loglik": float(best_fit.summary_loglik),
        "bic": _artifact_float("bic"),
        "classic_bic": float(
            best_classic_bic if best_classic_bic is not None else np.nan
        ),
        ...
        "selection_score_name": str(
            _artifact_value("selection_score_name", None) or selection_score
        ),
        "lambda_search_mode": str(selection_result.lambda_search_mode),
        "lambda_path_prespecified": _selected_bool(
            "lambda_path_prespecified", lambda_grid is not None
        ),
        ...
        "initializer_score_value": float(
            _selected_value("initializer_score_value", np.nan)
        ),
        ...
        "fusion_graph_likelihood_noise_degree_exponent": float(
            _selected_value("fusion_graph_likelihood_noise_degree_exponent", np.nan)
        ),
        ...
        "initializer_capacity_status": _selected_str("initializer_capacity_status", ""),
        ...
        "adaptive_refinement_rounds_completed": int(
            selection_result.adaptive_refinement_rounds_completed
        ),
        "adaptive_candidate_budget": _selected_int("adaptive_candidate_budget", -1),
        "adaptive_initial_anchor_count": _selected_int(
            "adaptive_initial_anchor_count", -1
        ),
        "adaptive_transition_probe_max_candidates": _selected_int(
            "adaptive_transition_probe_max_candidates",
            -1,
        ),
        "likelihood_partition_pool_enabled": _selected_bool(
            "likelihood_partition_pool_enabled", False
        ),
        "selection_elapsed_seconds": _timing_max("selection_elapsed_seconds"),
        "selection_prepare_elapsed_seconds": _timing_max(
            "selection_prepare_elapsed_seconds"
        ),
        "raw_candidate_elapsed_seconds": _timing_sum(
            "candidate_elapsed_seconds", raw_candidate_mask
        ),
        "raw_fit_elapsed_seconds": _timing_sum(
            "raw_fit_elapsed_seconds", raw_candidate_mask
        ),
        "raw_bic_refit_elapsed_seconds": _timing_sum(
            "bic_refit_elapsed_seconds", raw_candidate_mask
        ),
        ...
        "partition_generation_elapsed_seconds": _timing_max(
            "partition_generation_elapsed_seconds"
        ),
        "partition_curvature_elapsed_seconds": _timing_max(
        ...
        "selection_used_convergence_fallback": bool(
            selection_result.selection_used_convergence_fallback
        ),
        ...
        "num_candidates_all": int(
            getattr(
                selection_result, "num_candidates_all", selection_result.num_candidates
            )
        ),
        "num_candidates_certified": int(
            getattr(
                ...
                selection_result.num_converged_candidates,
            )
        ),
        ...
        if getattr(selection_result, "best_score_all_evaluated_kkt_residual", None)
        is None
        else float(selection_result.best_score_all_evaluated_kkt_residual),
        "best_score_all_evaluated_selection_eligible": bool(
            getattr(
                selection_result, "best_score_all_evaluated_selection_eligible", False
            )
        ),
        ...
        "selected_ari": np.nan
        if reported_selected_ari is None
        else float(reported_selected_ari),
        "selected_ari_source": selected_ari_source,
        "selected_ari_lambda": np.nan
        if reported_selected_ari is None
        else selected_lambda_value,
        "selected_ari_matches_selected_lambda": bool(
            reported_selected_ari is not None and selected_lambda_applicable
        ),
        "best_ari": np.nan
        if simulation_diagnostics.best_ari is None
        else float(simulation_diagnostics.best_ari),
        ...
        "selection_optimizer_limited": bool(
            getattr(selection_result, "selection_optimizer_limited", False)
        ),
        ...
        "best_converged_lambda_count": int(
            simulation_diagnostics.best_converged_lambda_count
        ),
        ...
        "ari_optimal_lambda_count": int(
            simulation_diagnostics.ari_optimal_lambda_count
        ),
        ...
        "adaptive_search_rounds_completed": int(
            selection_result.adaptive_search_rounds_completed
        ),
        "adaptive_search_stop_reason": str(
            selection_result.adaptive_search_stop_reason
        ),
        ...
        "adaptive_weight_baseline": float(
            _selected_value(
                "adaptive_weight_baseline", fit_options.adaptive_weight_baseline
            )
        ),
        "tol": float(_selected_value("tol", fit_options.tol)),
        "outer_max_iter": int(
            _selected_value("outer_max_iter", fit_options.outer_max_iter)
        ),
        "inner_max_iter": int(
            _selected_value("inner_max_iter", fit_options.inner_max_iter)
        ),
        ...
        "classic_bic_active_df_depth_n": _artifact_float(
            "classic_bic_active_df_depth_n"
        ),
        ...
        "bic_partition_max_diameter": float(
            _selected_value("bic_partition_max_diameter", np.nan)
        ),
        "bic_partition_diameter_exact": bool(
            _selected_value("bic_partition_diameter_exact", False)
        ),
        ...
        "bic_refit_finite_coordinate_count": _artifact_int(
            "bic_refit_finite_coordinate_count"
        ),
        ...
        "bic_refit_total_candidate_basins": _artifact_int(
            "bic_refit_total_candidate_basins"
        ),
        "bic_refit_total_refined_candidates": _artifact_int(
            "bic_refit_total_refined_candidates"
        ),
        "bic_refit_min_best_second_loss_gap": _artifact_float(
            "bic_refit_min_best_second_loss_gap"
        ),
        ...
        "bic_refit_phi_source": _selected_str(
        ...
        "objective_spread_across_starts": float(
            best_fit.objective_spread_across_starts
        ),
        ...
        "inner_backend": str(best_fit.inner_backend),
        "backend_iterations": int(best_fit.backend_iterations),
        "quotient_iterations": int(best_fit.quotient_iterations),
        "workset_iterations": int(best_fit.workset_iterations),
        "workset_expansions": int(best_fit.workset_expansions),
        "streamed_edge_passes": int(best_fit.streamed_edge_passes),
        "dense_iterations": int(best_fit.dense_iterations),
        "work_counter_scope": "selected_start_only",
        "certificate_iterations": int(best_fit.certificate_iterations),
        "activity_passes": int(best_fit.activity_passes),
        "analytic_adjoint_passes": int(best_fit.analytic_adjoint_passes),
        "column_scan_passes": int(best_fit.column_scan_passes),
        "full_certificate_audit_passes": int(best_fit.full_certificate_audit_passes),
        "fallback_reason": str(best_fit.fallback_reason),
        "inner_iterations": int(best_fit.inner_iterations),
        "admm_iterations": int(best_fit.admm_iterations),
        "exactness_provenance_version": int(best_fit.exactness_provenance_version),
        "estimator_role": str(best_fit.estimator_role),
        "objective_faithful": bool(best_fit.objective_faithful),
        "objective_spec_hash": str(best_fit.objective_spec_hash),
        "original_graph_hash": str(best_fit.original_graph_hash),
        "certificate_problem_hash": str(best_fit.certificate_problem_hash),
        "certificate_scope": str(best_fit.certificate_scope),
        "certificate_gradient_scope": str(best_fit.certificate_gradient_scope),
        "full_kkt_certified": bool(best_fit.full_kkt_certified),
        "full_kkt_certificate_status": str(best_fit.full_kkt_certificate_status),
        "full_kkt_tolerance": float(best_fit.full_kkt_tolerance),
        ...
        "last_attempted_inner_kkt_residual": float(
            best_fit.last_attempted_inner_kkt_residual
        ),
        "best_attempted_inner_kkt_residual": float(
            best_fit.best_attempted_inner_kkt_residual
        ),
        ...
        "outer_edge_subgradient_residual": float(
            best_fit.outer_edge_subgradient_residual
        ),
        ...
        "outer_num_lower_active_coordinates": int(
            best_fit.outer_num_lower_active_coordinates
        ),
        "outer_num_upper_active_coordinates": int(
            best_fit.outer_num_upper_active_coordinates
        ),
        ...
        "final_relative_objective_change": float(
            best_fit.final_relative_objective_change
        ),
        ...
        "raw_kkt_eligible": bool(
            _selected_value("raw_kkt_eligible", best_fit.selection_eligible)
        ),
        ...
        "selection_eligible": bool(
            _selected_value("selection_eligible", best_fit.selection_eligible)
        ),
        ...
        else float(best_evaluation.cp_rmse),
        "raw_cp_rmse": np.nan
        if best_evaluation is None
        ...
        if best_evaluation is None
            ...
            getattr(best_evaluation, "summary_cp_rmse", best_evaluation.cp_rmse)
        ),
        "multiplicity_f1": np.nan
        if best_evaluation is None
        else float(best_evaluation.multiplicity_f1),
        ...
        if best_evaluation is None
        else int(best_evaluation.n_eval_mutations),
        "n_filtered_mutations": np.nan
        if best_evaluation is None
        else int(best_evaluation.n_filtered_mutations),
        "bic_refit_ari": np.nan
        if (
            best_evaluation is None
            or getattr(best_evaluation, "bic_refit_ari", None) is None
        )
        else float(best_evaluation.bic_refit_ari),
        "bic_refit_cp_rmse": np.nan
        if (
            best_evaluation is None
            or getattr(best_evaluation, "bic_refit_cp_rmse", None) is None
        )
